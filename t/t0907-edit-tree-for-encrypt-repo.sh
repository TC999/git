...
# If compiled with git encrypt/decrypt support, set prereq.
if test_expect_code 129 git decrypt -h >/dev/null 2>&1
then
	test_set_prereq AGIT_CRYPTO
fi

test_expect_success AGIT_CRYPTO "setup encrypted repo" '
...
test_expect_success AGIT_CRYPTO "ls-tree root tree" '
	cat >actions <<-\EOF &&
		ls-tree
	EOF
...
test_expect_success AGIT_CRYPTO "ls-tree subdir" '
	cat >actions <<-\EOF &&
		# show subtree: 1
		ls-tree 1
...
test_expect_success AGIT_CRYPTO "walk existing subdir" '
	cat >actions <<-\EOF &&
		walk 1/12/124
		ls-tree
	EOF
...
test_expect_success AGIT_CRYPTO "walk non-exist tree to create placeholders" '
	cat >actions <<-\EOF &&
...
test_expect_success AGIT_CRYPTO "rm file" '
	cat >actions <<-\EOF &&
		rm 1/12/123/foo.1
		ls-tree
	EOF
...
test_expect_success AGIT_CRYPTO "rm: fail to remove non-exist file" '
	cat >actions <<-\EOF &&
		rm 1/12/123/missing
		ls-tree
	EOF
...
test_expect_success AGIT_CRYPTO "rm non-exist file by force" '
	cat >actions <<-\EOF &&
...
test_expect_success AGIT_CRYPTO "rm non-exist file in non-exist dir by force" '
	cat >actions <<-\EOF &&
...
test_expect_success AGIT_CRYPTO "rm subdir" '
	cat >actions <<-\EOF &&
		rm 1/12/123
		ls-tree
	EOF
...
test_expect_success AGIT_CRYPTO "add file in root tree" '
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "add file in sub tree" '
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "add duplicate file: show warning" '
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "add duplicate file: show error for different mode" '
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "add duplicate file: force to replace" '
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "add duplicate tree: show warning" '
...
test_expect_success AGIT_CRYPTO "add duplicate tree: show error for different OID" '
...
test_expect_success AGIT_CRYPTO "add duplicate tree: force to overwrite a tree with different OID" '
	oid_tree=$(git -C repo.git rev-parse "HEAD:2") &&
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "add a file but the object is missing" '
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "add a missing commit as submodule is OK" '
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "fatal: add a blob as a tree" '
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "fatal add a tree object as a file" '
...
test_expect_success AGIT_CRYPTO "add file into a non-exist dir" '
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "add tree into a non-exist parent dir" '
...
test_expect_success AGIT_CRYPTO "replace file mode" '
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "replace blob object" '
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "replace tree" '
...
test_expect_success AGIT_CRYPTO "fail to replace a missing object" '
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "replace a missing object, will add one" '
	cat >actions <<-EOF &&
...
test_expect_success AGIT_CRYPTO "commit several times" '
	cat >actions <<-EOF &&
